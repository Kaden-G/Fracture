...
function moveRange(fk){ return G.factions[fk]?.trait==='ghost_step' ? 2 : 1; }  // 👻 GHOST STEP = reach 2
...
function moveReachable(fk, src, dest){
  ...
  const t=G.factions[fk]?.trait;
  if (t!=='ghost_step') return false;                        // only GHOST STEP reaches 2
  // 2 steps: need an intermediate tile M adjacent to both src and dest
  return Object.values(G.tiles).some(M =>
    M.id!==src.id && M.id!==dest.id && adjacent(src,M) && adjacent(M,dest)
  );  // 👻 GHOST STEP: slip through any tile
}
...
function startGame() {
  ...
  if (G.setup.tyrant) {
    factions[TYRANT_KEY] = mkFaction(TYRANT_DEF.name, TYRANT_KEY, true, randTrait());
...
function setAction(action) {
  ...
  const hints = {
    ...
    sabotage:  'SABOTAGE (1 res): Click any ENEMY tile to remove 1 troop.',
...
function handleTileClick(id) {
  ...
  if (currentAction === 'move') {
    ...
    if (!moveReachable(G.playerFaction, src, tile)) {
      ...
      setActionLog(t==='ghost_step' ? 'Out of range — max 2 tiles.' : 'Not adjacent! Try again.');
...
function buildOnlineGame(seats, tyrant) {
  ...
  const turnOrder = order.slice();
  if (tyrant) { factions[TYRANT_KEY] = mkFaction(TYRANT_DEF.name, TYRANT_KEY, true, randTrait()); turnOrder.push(TYRANT_KEY); }
  G = {
